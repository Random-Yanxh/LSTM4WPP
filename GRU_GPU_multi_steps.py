...
PREDICTION_STEPS = 16
WINDOW_SIZE = 96
GRU_HIDDEN_SIZE = 256  # GRU隐藏层大小 (If OOM, consider reducing to 128)
GRU_NUM_LAYERS = 3   # GRU层数 (If OOM, consider reducing to 2)
GRU_DROPOUT = 0.3
FC_LAYERS = [128, 32]
...
MODEL_SAVE_PATH = "./gru_wpp_model_gpu.pth" # Changed model save path for GRU version
FIGURE_SAVE_DIR = "./figures_gru_gpu" # Changed figure save dir for GRU version
...
# III. 改进的GRU模型定义 (PyTorch nn.Module)
class AttentionLayer(nn.Module):
    ...
    def forward(self, rnn_output): # Parameter name changed for generality
...
class ImprovedGRUModel(nn.Module):
    def __init__(self, input_features: int, hidden_size: int, num_layers: int,
        ...
        # 注意力机制
        self.attention = AttentionLayer(hidden_size) # hidden_size for unidirectional

        self.dropout = nn.Dropout(dropout_rate)

        # 全连接层
        fc_module_list = []
        current_dim = hidden_size # hidden_size for unidirectional
        if fc_layers_config:
        ...
        print("ImprovedGRUModel initialized.")
        print(f"  Input features: {input_features}")
        print(f"  Hidden size: {hidden_size}, Num layers: {num_layers}, Bidirectional: False")
        print(f"  GRU Dropout: {dropout_rate if num_layers > 1 else 0}")
...
class GRUModel(nn.Module):
    def __init__(self, input_features: int, hidden_size: int, num_layers: int,
        ...
        print("GRUModel initialized.")
        print(f"  Input features: {input_features}")
        print(f"  Hidden size: {hidden_size}, Num layers: {num_layers}, GRU Dropout: {dropout_rate if num_layers > 1 else 0}")
        print(f"  FC Layers Config: {fc_layers_config}")
        print(f"  Output steps: {output_steps}")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = gru_out[:, -1, :] # Take the output of the last time step
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader,
    ...
    epochs_no_improve = 0
    best_model_temp_path = model_save_path.replace(".pth", "_temp.pth") # Generic temp path
...
def calculate_cr_accuracy(P_M, P_P): # noqa: N802
    ...
    if P_M.shape != P_P.shape:
        raise ValueError(f"P_M shape: {P_M.shape}, P_P shape: {P_P.shape}")
...
if __name__ == "__main__":
    print("--- Wind Power Prediction using GRU (GPU Optimized Version) ---")
    ...
    print("\n--- Initializing Model ---")
    gru_model = ImprovedGRUModel( # Changed from lstm_model to gru_model
        input_features=1,
        hidden_size=GRU_HIDDEN_SIZE, # Changed from LSTM_HIDDEN_SIZE
        num_layers=GRU_NUM_LAYERS,   # Changed from LSTM_NUM_LAYERS
        dropout_rate=GRU_DROPOUT,    # Changed from LSTM_DROPOUT
    ...
    optimizer = optim.AdamW(gru_model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4) # Changed to gru_model
    ...
    train_loss_hist, val_loss_hist = train_model(
        model=gru_model, # Changed to gru_model
    ...
    network_params_str = (f"GRU(H:{GRU_HIDDEN_SIZE}, L:{GRU_NUM_LAYERS}, D:{GRU_DROPOUT if GRU_NUM_LAYERS > 1 else 0}), " # Changed LSTM to GRU
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        gru_model.load_state_dict(torch.load(MODEL_SAVE_PATH, map_location=DEVICE)) # Changed to gru_model
