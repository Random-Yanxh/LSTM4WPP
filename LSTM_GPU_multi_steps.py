...
LSTM_NUM_LAYERS = 3  # 增加LSTM层数 (If OOM, consider reducing to 2)
LSTM_DROPOUT = 0.3
...
MODEL_SAVE_PATH = "./lstm_wpp_model_gpu.pth" # Changed model save path for GPU version
FIGURE_SAVE_DIR = "./figures_gpu" # Changed figure save dir for GPU version
...
class ImprovedLSTMModel(nn.Module):
    def __init__(self, input_features: int, hidden_size: int, num_layers: int,
        ...
        self.lstm = nn.LSTM(input_size=input_features,
                            ...
                            bidirectional=False) # Changed to False for unidirectional
...
if __name__ == "__main__":
    print("--- Wind Power Prediction using LSTM (GPU Optimized Version) ---")
    ...
    train_loss_hist, val_loss_hist = train_model(
        ...
        device=DEVICE,
        model_save_path=MODEL_SAVE_PATH, # Uses the GPU specific model path
        scheduler=scheduler
    )
    ...
    if os.path.exists(MODEL_SAVE_PATH): # Uses GPU specific model path
