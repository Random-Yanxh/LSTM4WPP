...
CNN_DROPOUT = 0.2                # CNN Dropout率

# LSTM 相关参数
LSTM_HIDDEN_SIZE = 256  # 增加隐藏层大小 (If OOM, consider reducing to 128)
LSTM_NUM_LAYERS = 2     # 减少LSTM层数以适应CNN-LSTM结构
LSTM_DROPOUT = 0.3

FC_LAYERS = [128, 64, 32]  # 增加全连接层
BATCH_SIZE = 32  # 减小批次大小以适应更大的模型 (If OOM, reduce to 16 or 8)
LEARNING_RATE = 0.0005  # 降低学习率
MAX_EPOCHS = 100 # 增加训练轮数
EARLY_STOPPING_PATIENCE = 15
EARLY_STOPPING_MIN_DELTA = 0.0001
NORMALIZATION_METHOD = "standard"  # 改用标准化
MODEL_SAVE_PATH = "./cnn_lstm_wpp_model_gpu.pth" # CNN-LSTM模型保存路径
FIGURE_SAVE_DIR = "./figures_cnn_lstm" # CNN-LSTM图片保存目录
...
def set_seed(seed_value):
    ...
    # CUDA specific seed settings
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed_value)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = True # Set to True for potential speedup if input sizes are consistent
...
# III. CNN-LSTM混合模型定义 (PyTorch nn.Module)
class CNNFeatureExtractor(nn.Module):
    """CNN特征提取器"""
    def __init__(self, input_features: int, num_filters: list, kernel_sizes: list, dropout_rate: float):
        super(CNNFeatureExtractor, self).__init__()
        self.conv_layers = nn.ModuleList()

        # 创建多个不同kernel size的卷积层
        for i, (num_filter, kernel_size) in enumerate(zip(num_filters, kernel_sizes)):
            conv_block = nn.Sequential(
                nn.Conv1d(in_channels=input_features if i == 0 else num_filters[i-1],
    ...
    def forward(self, x):
        ...
        # 转换为CNN期望的格式: (batch_size, input_features, seq_len)
        x = x.permute(0, 2, 1)

        for conv_layer in self.conv_layers:
            x = conv_layer(x)

        # 转换回LSTM期望的格式: (batch_size, seq_len, features)
        x = x.permute(0, 2, 1)
        return x
...
class CNNLSTMModel(nn.Module):
    def __init__(self, input_features: int, cnn_num_filters: list, cnn_kernel_sizes: list,
                 cnn_dropout: float, lstm_hidden_size: int, lstm_num_layers: int,
                 lstm_dropout: float, fc_layers_config: list, output_steps: int):
        ...
        # LSTM层 - 输入特征数为CNN输出通道数
        self.lstm = nn.LSTM(input_size=self.cnn_extractor.output_channels,
        ...
        if fc_layers_config:
            for fc_hidden_dim in fc_layers_config:
                ...
                fc_module_list.append(nn.Dropout(lstm_dropout))
        ...
        print("CNNLSTMModel initialized.")
        print(f"  Input features: {input_features}")
        print(f"  CNN filters: {cnn_num_filters}, kernel sizes: {cnn_kernel_sizes}")
        print(f"  CNN dropout: {cnn_dropout}")
        print(f"  LSTM hidden size: {lstm_hidden_size}, num layers: {lstm_num_layers}")
        print(f"  LSTM dropout: {lstm_dropout if lstm_num_layers > 1 else 0}")
        print(f"  FC layers: {fc_layers_config}")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 初始化LSTM隐藏状态
        h0 = torch.zeros(self.lstm_num_layers, batch_size, self.lstm_hidden_size, device=x.device)
        c0 = torch.zeros(self.lstm_num_layers, batch_size, self.lstm_hidden_size, device=x.device)
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader,
    ...
    epochs_no_improve = 0
    best_model_temp_path = 'best_model_temp_gpu.pth' # Temp path for GPU version
...
if __name__ == "__main__":
    print("--- Wind Power Prediction using CNN-LSTM Hybrid Model (GPU Optimized Version) ---")
    ...
    # 使用加权损失函数
    criterion = WeightedMSELoss(prediction_steps=PREDICTION_STEPS, weight_decay=0.85).to(DEVICE) # Ensure criterion weights are on device

    # 使用AdamW优化器，添加权重衰减
    optimizer = optim.AdamW(cnn_lstm_model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4)
    ...
    network_params_str = (f"CNN-LSTM(CNN:{CNN_NUM_FILTERS}, LSTM_H:{LSTM_HIDDEN_SIZE}, LSTM_L:{LSTM_NUM_LAYERS}), "
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        cnn_lstm_model.load_state_dict(torch.load(MODEL_SAVE_PATH, map_location=DEVICE))
